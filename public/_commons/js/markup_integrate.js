	...
	*/
	...
	var preHTMLBlock = {
		...
		run: function() {
			...
			var attr = m[1];
      // markdownでpre付きになっているので、エスケープしない
			//var code = this.escapeHTML(m[2]);
			//this.out += '<pre'+attr+'>'+code+'</pre>';
			this.out += '<pre'+attr+'>'+m[2]+'</pre>';
		}
	};
...
function get_markuped(src, opt, callback){
    var markuped_val = '';
    // 最初にmarkdownを適用
    markuped_val = marked(src).replace(/&gt;/g,'>');
...
if (typeof module !== 'undefined' && typeof exports === 'object') {
  ...
  // ブラウザの場合はここで作ったメソッドを登録
  this.marked = marked;
  this.get_markuped = get_markuped;
}
